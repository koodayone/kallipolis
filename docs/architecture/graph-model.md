...
| `DEMANDS` | Region → Occupation | employment, annual_wage, growth_rate, annual_openings, vintage | A region has demand for an occupation, with the regional employment, wage, growth, and openings metadata that varies by region. `vintage` names the Centers of Excellence release the four figures come from (the bundled export's base year and projection window), so the graph states its own edition and a graph that lags the bundled file is detectable without comparing values |
