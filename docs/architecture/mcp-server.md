...
The catalog is deliberately incomplete, and the missing cells of the grid are the roadmap. The largest are the sector-anchor aggregate as a standalone orientation form and cross-institution comparison — ranking a set of colleges against each other on supply share and coverage, framed as positioning rather than competition. These are not new primitives; they are the same relation evaluated across a set of colleges rather than a single anchor, and they extend the existing comparison engine with a new unit type. A new form is earned by evidence that the catalog cannot answer a real question, not by enumeration in advance.
