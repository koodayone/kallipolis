...
The consortium pathway report (`backend/partnerships/report.py`) carries a further section when a curriculum-alignment roster has been run for it. It reads by occupation: for each of the role's occupations, the ten most important core work activities O*NET records for it, each followed by the courses across the consortium whose outlines of record evidence it, as chips in the college's colour. A solid chip means a course outcome or objective states the activity; an outlined chip means the course's description, content, lab or assignments involve it; an activity with no chip is one no member college evidences. A readout under each occupation counts what the consortium covers, names the colleges carrying the most evidence, and lists the uncovered activities. Five programs converge on three occupations, so this is the view that shows the shared core and the specializations at once; the same saved alignment also renders as a per-program plate for a single college's own report.
