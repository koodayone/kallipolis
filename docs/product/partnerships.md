...
The consortium pathway report (`backend/partnerships/report.py`) carries a further section when a curriculum-alignment roster has been run for it. It reads by occupation: for each of the role's occupations, the ten most important core work activities O*NET records for it, each followed by the courses across the consortium whose outlines of record evidence it, as chips in the college's colour. A chip means the course's outline evidences the activity, in an outcome, an objective, or the course's content; the appendix says which. An activity with no chip is one the outlines read so far do not evidence, and the report leaves it unannotated: a chip is a checked sentence, while absence of evidence is the method's weakest claim and one for the college to confirm before it is called a gap. A readout under each occupation counts what the consortium covers and names the colleges carrying the most evidence. Five programs converge on three occupations, so this is the view that shows the shared core and the specializations at once; the same saved alignment also renders as a per-program plate for a single college's own report.
