...
## The outline of record

Every credit and noncredit course at a California community college has a Course Outline of Record: the board-approved statement of the course's description, student learning outcomes, objectives, content, methods of instruction and evaluation, and assignments, required by Title 5 of the California Code of Regulations. The catalog description is a summary of this document. The outline is the document itself, and it is the form of the course Kallipolis reads when it asks what a course prepares students to do.

The outline matters because of its grammar. Learning outcomes and objectives are written as statements of what students will be able to do; lab content is a list of things students do. That is the same altitude as the work activities O*NET records for an occupation, so an outline can be read against an occupation's activities sentence by sentence, where a catalog blurb cannot. Read from a marketing description, Foothill College's semiconductor certificate showed four weak marks against its target occupation; read from its outlines it showed ten.

There is no statewide repository of outline text. The Chancellor's Office Curriculum Inventory holds the approval record and control codes for every course but not its outcomes or content. The outline lives in each college's curriculum management system, and Kallipolis retrieves it through one adapter per vendor (`backend/courses/outlines.py`): CourseLeaf pages for Foothill and, under a heading profile of its own, for Orange Coast (the Coast district catalog prints each outline under the state's field names); CurricUNET outline reports for Ohlone; eLumen's public curriculum view for Mission and De Anza; and CurriQunet reports for Evergreen Valley and San Diego Mesa, where the district's public catalog carries each course's report id so no scan is needed. Each retrieved outline carries the public link a reader follows to check a quoted sentence at its source, and a link check (`python -m courses.outlines check`) fetches every cached link, rendering the single-page systems, and records whether the page names the course; the verdicts are pinned by the test suite. Where a system cannot be deep-linked to an outline, the link goes to the best public page: De Anza's eLumen catalog page for the outline's own catalog year, which embeds the full outline, and Mission's public curriculum view, where the outline is two selections in. Retrieved outlines are cached as JSON under `backend/courses/data/outlines/` with the URL a reader can open, the dates the college printed on the outline, and a note of which sections that college's format carries. Each outline's currency is checked against the Curriculum Inventory's record of when the college last updated the course (`backend/ontology/coci.py`): an outline older than that record may have a newer version in the college system.

The formats differ in what they carry. Three of the five SVAMP colleges publish course objectives; Ohlone folds objectives into its outcomes; Evergreen Valley's objectives field holds only the form's template text. The alignment reads whatever sections a college's outline has and says so on the plate, rather than treating the formats as interchangeable.
...
Two improvements would strengthen the curriculum side without requiring a redesign. The first is wider outline coverage. The outline of record is now the source for the five SVAMP colleges' programs; extending the vendor adapters to every college the ontology carries would let any program be read against its target occupation the same way. Direct access to learning outcome assessments and faculty-described course content would add a further layer beyond the outline.
