...
**3. Assign skills.** `backend/occupations/assign_skills.py` loads `occupations.json`, batches the occupations, and sends them to `gemini-2.5-flash` with the full unified skills taxonomy interpolated into the system instruction as a closed vocabulary. The system instruction asks for 6–10 skills per occupation, prioritizing primary technical skills and core professional competencies, and explicitly excluding adjacent or tangential skills. The 6–10 band replaced an earlier "at least 6, be thorough" framing that produced a mode-on-floor distribution under thin descriptions and asymmetric inflation under rich ones; the bounded range produces a tight distribution (median 7, P95 of 9) that doesn't depend on description verbosity. Each batch carries, for every occupation, the SOC code, title, description, and education level, so the model can distinguish e.g. "Registered Nurses (Bachelor's degree)" from "Licensed Practical and Licensed Vocational Nurses (Postsecondary nondegree award)" rather than working from titles alone. The batch size is `BATCH_SIZE = 20` and Gemini calls run under `CONCURRENCY = 10` with exponential-backoff retries on `resource_exhausted` errors up to `MAX_RETRIES = 5`.
...
**4. Load.** `backend/occupations/load.py` writes the final `occupations.json` into Neo4j. The loader creates or matches `Region` nodes for every distinct region key observed, links each college to its primary COE region via `IN_MARKET`, creates or merges an `Occupation` node per SOC code with `title`, `description`, and `education_level` set on the node, creates one `DEMANDS` edge per (region, occupation) pair carrying the four regional-metric properties, drops every existing `REQUIRES_SKILL` edge in the graph, and rebuilds them from the current mapping. All writes use `UNWIND`-driven `MERGE` in batches of `BATCH_SIZE = 500`. The blanket `REQUIRES_SKILL` clear before the rebuild is a deliberate choice: `MERGE` is idempotent for additions but does not handle removals, so without the clear a re-run after `assign_skills.py` would layer new skills on top of stale ones rather than reflecting the current mapping. Skill nodes themselves are left alone — they may still be referenced by `Course-[:DEVELOPS]->Skill` edges and are shared across the graph.
...
**The 6–10 cap is a count discipline, not a tier signal.** `REQUIRES_SKILL` edges are flat — they carry no `weight` or `tier` property — so Course→Skill→Occupation matching in the partnership pipeline counts every edge equally. The bounded skill range keeps the count from drifting with description verbosity, but it does not address the underlying signal-dilution question of whether an occupation's most-distinctive skills should weigh more than its general ones. That distinction would require a schema change (tier or weight on the edge) and corresponding aggregation in `backend/employers/api.py` matching Cypher; the current architecture treats it as a downstream design question to evaluate after observing partnership matching behavior under the bounded skill counts.
