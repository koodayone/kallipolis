...
**1. Parse.** `generate.py` reads the COE middle-skill CSV, coerces each row's employment/wage/growth/openings cells into typed values, and pivots the rows into per-occupation records. No filter runs here: the source file is the curated set, so the stage is 1:1 with it. The export's columns are resolved from its header by pattern (`ontology.supply.coe_demand_columns`), because each COE release renames the year-bearing columns and a reader that named them would produce nulls without an error; a column no pattern matches stops the run. Beside the generated file the stage writes `backend/occupations/occupations.meta.json`, naming the export it came from and that export's vintage, so a generated file that lags the bundled export is detectable without comparing values. The loader stamps the same vintage on every `DEMANDS` edge for the same reason.
