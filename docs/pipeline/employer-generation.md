...
`generate.py` produces `employers.json`. A separate script, `backend/employers/load.py`, loads it into Neo4j. The loader creates one `Employer` node per record, links it to each region in its `regions` array via `IN_MARKET`, and creates two parallel occupation edge sets. `HIRES_FOR` is derived deterministically from `oes_socs_for_naics4(employer.naics4)`: every SOC the BLS OEWS National Industry-Occupation Matrix publishes at `pct_total > 0` for the employer's `naics4` becomes an edge, with the published share persisted as the `pct_total` edge property. `IDENTITY_HIRES_FOR` is a parallel overlay carrying the 3–8 LLM-curated SOCs from the employer's `occupations` array, marking which roles the enrichment pass found most characteristic of this specific employer. The `swp_sectors` node property is also set deterministically by the loader from `CTE_NAICS_CODES[naics4][2]`, so sector tagging is sourced from the same NAICS-keyed authority the rest of the graph uses. Loading is idempotent: re-running prunes stale edges of both types, then MERGEs the current set, so the graph matches the current `employers.json` exactly without legacy residue. A companion helper, `prune_region_in_market()`, removes stale `IN_MARKET` edges when a region's employer pool is re-scraped and some prior entries no longer survive the new run.
