...
Between stages 2 and 3, a department-canonicalization pass rewrites the `department` field on each course from Gemini's fragmented output to the canonical display name computed from the course code's prefix via a committed mapping at `backend/courses/department_mapping/`. The detailed treatment, including the seeder tool and the post-load cleanup the Neo4j loader runs to converge stale department state, is in [Course Generation](./course-generation.md).
