...
function LmiSection({ lmi, brandColor }: { lmi: ApiLmiContext; brandColor: string }) {
  ...
  const lmiText = [
    ...
    ...lmi.occupations.map(o =>
      `  ${o.soc_code} ${o.title} — ${o.region}: ${o.annual_wage ? `$${o.annual_wage.toLocaleString()}/yr` : "wage N/A"}, ${o.employment ? `${o.employment.toLocaleString()} jobs` : "employment N/A"}`
    ),
    "",
    "Supply (3-year avg completions):",
    ...lmi.supply_estimates.map(s =>
      `  ${s.top_title} (${s.department}): ${s.estimated_annual_completions.toLocaleString()}`
    ),
    "",
    `Total Demand: ${lmi.total_demand.toLocaleString()}`,
    `Total Supply: ${lmi.total_supply.toLocaleString()}`,
    `Gap: ${lmi.gap > 0 ? "+" : ""}${lmi.gap.toLocaleString()}`,
  ...
  return (
    <div style={{
        ...
        <tbody>
          {lmi.occupations.map((occ, i) => (
            <tr key={i}>
              ...
              <td style={tCellStyle}>{occ.employment ? occ.employment.toLocaleString() : "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div style={{ fontFamily: FONT, fontSize: "12px", color: "rgba(255,255,255,0.55)", lineHeight: 1.8 }}>
        <span style={{ fontWeight: 500 }}>Total Demand:</span> {lmi.total_demand.toLocaleString()}
        {" · "}
        <span style={{ fontWeight: 500 }}>Total Supply:</span> {lmi.total_supply.toLocaleString()}
        {" · "}
        <span style={{ fontWeight: 500 }}>Gap:</span>{" "}
        <span style={{ color: lmi.gap_eligible ? "rgba(74,222,128,0.8)" : "rgba(248,113,113,0.8)" }}>
          {lmi.gap > 0 ? "+" : ""}{lmi.gap.toLocaleString()}
        </span>
      </div>
    </div>
  );
