...
type ClickState =
  | { phase: "idle" }
  | { phase: "pulse"; elapsed: number; node: IndustryNodeKey }
  | { phase: "dissolve"; elapsed: number; node: IndustryNodeKey };
...
export function buildIndustryScene(
  ...
  // ── Interaction State ─────────────────────────────────────────────────────
  let hoveredNode: IndustryNodeKey | null = null;
  let clickState: ClickState = { phase: "idle" };
  let selectedNode: IndustryNodeKey | null = null;
  let isLocked = false;
  ...
  function handleHover(node: IndustryNodeKey | null) {
  ...
  function handleClick(node: IndustryNodeKey) {
    if (isLocked) return;
    isLocked = true;
    selectedNode = node;
  ...
  function tick() {
    ...
    if (clickState.phase === "pulse") {
      ...
      const pulsingSolid = solids.find((s) => s.node === (clickState as { node: IndustryNodeKey }).node);
      if (pulsingSolid) {
      ...
      const selectedSolid = solids.find((s) => s.node === (clickState as { node: IndustryNodeKey }).node);
      if (selectedSolid) {
      ...
      if (clickState.elapsed >= 0.7) {
        callbacks.onNodeClick((clickState as { node: IndustryNodeKey }).node);
    ...
    solids.forEach((s) => {
      ...
      if (clickState.phase !== "pulse" || (clickState as { node: IndustryNodeKey }).node !== s.node) {
