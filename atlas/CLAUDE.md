...
When you change code under `college-atlas/` or `state-atlas/`, run `npm test` from `atlas/` to exercise the Vitest unit suite. Tests colocate with their source files (e.g. `buildSwpRequest.test.ts` next to `buildSwpRequest.ts`). When adding a new test file, start it with a JSDoc coverage header per [`atlas/docs/testing.md`](./docs/testing.md) — the docs audit enforces the convention and will block merge without it.
